...
const WEEKS = {
  ...
  4: {
    ...
    video: "",
    coach: "Week 4 is the first week students open Make. Theme of the day: 'Automate yourself' — automation protects your best hour, and the design is not finished until you decide what goes into the time it gives back. AI runs on your own thinking, and thinking comes from exercise, good places and good friends, so the student goes inside the automation. Remind them of this in one or two plain sentences when it fits, and ask one easy question from their own life (when is your best hour? what will you do with the time?) — do not lecture. Most students use make.com in the phone browser (the Make app only runs scenarios). Build on the Week 3 response sheet with the SAME Google account as Week 3, from the template 'Send a Gmail email from a new Google Sheets row' (Templates → Start guided setup): Google Sheets 'Watch New Rows' → Gmail 'Send an email'. The template asks for Email Address/Subject/Content columns — not needed; they type their own address in To. Connecting each account is the most important step: Add → Sign in with Google → same account → tick every permission box → Allow; Gmail needs its own connection. On '403 PERMISSION_DENIED — Method doesn't allow unregistered callers': (1) Credentials → delete that Google connection → create it again, checking the Google account screen really opens; (2) if choosing the file still fails, type the Spreadsheet ID (between /d/ and /edit in the sheet URL); (3) still stuck on the phone → connect on the PC. 'Run once' gives one email per answer — too many; then put Tools → 'Text aggregator' between them (source: Google Sheets, row separator: New row) and put its Text in the email, so all answers arrive in ONE email, with one last line under them that is only theirs: one thing for their body, or one person to contact today. Schedule Every day, Save, turn ON — every check costs a credit even when nothing is new (every 15 minutes = 2,880 a month against the free 1,000). No AI in this scenario on purpose; next week AI goes in one place. Ask what the answers say about their idea and one thing they will change or keep. The chat takes text only: ask them to paste the subject and first lines of the email Make sent. If a connection still fails, ask for the exact error and send them to the professor — do not invent workarounds.",
    links: [
    ...
    tutorial: `
      ...
      <p><b>Add</b> → <b>Sign in with Google</b> → the same account → <b>tick every box</b> → Allow. Gmail needs <b>its own</b> connection — do it again at the Gmail step.</p>
      <p><b>403 "Method doesn't allow unregistered callers"?</b> ☰ → Credentials → delete that connection → make it again. Choosing the file still fails? Type the Spreadsheet ID — the part of your sheet address between /d/ and /edit. Still stuck on the phone? Connect on the PC.</p>
      <h4>Step 3 — fill in, run once</h4>
      <p>Sheets: your Week 3 spreadsheet, sheet <b>Form Responses 1</b>, headers Yes (if asked where to start, choose All). Gmail: To your own address, Subject New answers, the email text: one column. Press <b>Run once</b> — one email per answer. Too many.</p>
      <h4>Step 4 — the point in the middle, your last line, the clock</h4>
      <p>Between them, add <b>Tools</b> → <b>Text aggregator</b> (source Google Sheets, row separator New row) and put its <b>Text</b> in the email. Under it, type one line that is only yours: one thing for your body, or one person to contact today.</p>
