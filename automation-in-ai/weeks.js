...
const WEEKS = {
  ...
  4: {
    ...
    video: "",
    coach: "Week 4: students open Make for the first time and build Google Sheets 'Watch New Rows' → a NEW Gmail 'Send an email', from zero (the template's Gmail module is old — 'restricted scopes'). Follow GUIDE MODE step by step. Theme 'Automate yourself' — one plain sentence at most; the class goes straight into practice. Connections only on a PC: on a phone the Google sign-in and permission window does not appear ('403 — Method doesn't allow unregistered callers' → Credentials → delete that connection → make it again on the PC). Tick every permission box → Continue; Gmail needs its own connection. CONTENT: give the whole text at once, built from THIS student's Week 3 survey questions (their earlier submission; if missing, ask them to read their sheet's column headers). First line <b>New answer</b><br>, then one line per question: a short label, a colon, the column letter in brackets, then <br>. Column A is the timestamp, so the first question is (B). Example: <b>New answer</b><br> / Problem: (B)<br> / Now they use: (C)<br> / Would pay: (D)<br> — one line each. Then: copy it all, click in Content, paste; for each letter select '(B)', delete it, leave the cursor there, click the green block that ends with (B); never the top [bundle] block (it sends the row as code); Save. Body type is Raw HTML, so Enter makes no new line — that is why <br>. Before Run once: right-click Sheets → Choose where to start → All. A second Run once sends nothing unless there is a new row. To: their own address + smit.ai.lab@gmail.com; Subject must start with W4 ('W4 · name · New answer'). Clock: toolbar 'Every 15 minutes' → Every day → Save → Activate scenario. No AI in this scenario. Submission: the server checks the professor's inbox when they type their Gmail address; the draft appears only after ARRIVED ✓ with W4 — never by their word. Do not ask for a last line or any extra line in the email. If a connection still fails, ask for the exact error and send them to the professor — do not invent workarounds.",
    links: [
    ...
    interviewLabels: [
      ...
      "smit.ai.lab@gmail.com added, and W4 in the Subject? (yes / no)",
    ...
    tutorial: `
      ...
      <p><b>Do the connections on the PC.</b> On a phone, the Google sign-in and permission window <b>does not appear</b> and you get <b>403 "Method doesn't allow unregistered callers"</b>. Then ☰ → Credentials → delete that connection → make it again on the PC. Your phone uses it after that.</p>
      ...
      <p><b>Content</b>: ask the AI assistant for your text — it writes your own questions, each with a letter, like <code>Problem: (B)&lt;br&gt;</code>. Copy it all → click in Content → paste. Then swap each letter: select <b>(B)</b>, delete it, leave the cursor there, click the green block that ends with <b>(B)</b>. Same for (C), (D)… Not the top <b>[bundle]</b> block. Column A is the time, so your first question is (B).</p>
      ...
      <p>☐ Subject starts with <b>W4</b> (W4 · your name · New answer)<br>☐ <b>smit.ai.lab@gmail.com</b> is in To<br>☐ A <b>new row</b> in your sheet, then <b>Run once</b><br>☐ The email arrived in <b>your</b> Gmail<br>☐ In the chat, type the Gmail address you connected → wait for <b>ARRIVED ✓</b> → press <b>Submit</b> on the draft → see it saved</p>
      <p>The draft does not appear until the professor's inbox shows <b>ARRIVED ✓</b>. No W4 in the Subject = not accepted.</p>
