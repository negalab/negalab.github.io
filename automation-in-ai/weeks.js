...
const WEEKS = {
  ...
  4: {
    ...
    video: "",
    coach: "Week 4 is the first week students open Make. Theme: 'Automate yourself' — automation protects your best hour, and the design is not finished until you decide what goes into the time it gives back; thinking comes from exercise, good places and good friends, so the student goes inside the automation. Remind them of this in a plain sentence when it fits and ask one easy question from their own life — do not lecture. Most use make.com in the phone browser (the Make app only runs scenarios). Build on the Week 3 response sheet with the SAME Google account, from the template 'Send a Gmail email from a new Google Sheets row' (Templates → Start guided setup): Google Sheets 'Watch New Rows' → Gmail 'Send an email'. The template's Email Address/Subject/Content columns are not needed; they type their own address in To. Connecting each account is the most important step: Add → Sign in with Google → same account → tick every permission box → Continue; Gmail needs its own connection. On '403 PERMISSION_DENIED — Method doesn't allow unregistered callers': (1) Credentials → delete that connection → create it again, checking the Google account screen really opens; (2) if choosing the file fails, in the editor set Search Method → Enter manually and paste the Spreadsheet ID (between /d/ and /edit); (3) still stuck on the phone → connect on the PC. Before 'Run once': right-click the Sheets module → Choose where to start → All → Save, or it finds nothing. Run once gives one email per answer — too many; then right-click the dots between them → Add a module → Tools → 'Text aggregator' (source: Google Sheets; Advanced settings toggle → row separator New row) and put its Text in the email, with one last line under it that is only theirs: one thing for their body, or one person to contact today. Schedule: toolbar 'Every 15 minutes' → Every day → Save → Activate scenario (every check costs a credit; every 15 minutes = 2,880 a month against the free 1,000). No AI in it on purpose; next week AI goes in one place. Ask what the answers say about their idea and one thing they will change or keep. The chat takes text only: ask them to paste the subject and first lines of the email Make sent. Make's own lessons teach the same build (help.make.com/create-your-first-scenario, help.make.com/expand-your-scenario — Step 6 is the Text aggregator). If a connection still fails, ask for the exact error and send them to the professor — do not invent workarounds.",
    links: [
    ...
    tutorial: `
      ...
      <p>☰ → <b>Templates</b> → search <b>Send a Gmail email from a new Google Sheets row</b> → <b>Start guided setup</b>. It asks for Email Address, Subject and Content columns — you do not need them. You type your own address.</p>
      <h4>Step 2 — connect each account (the most important step)</h4>
      <p><b>Add</b> → <b>Sign in with Google</b> → the same account → <b>tick every box</b> → <b>Continue</b>. Gmail needs <b>its own</b> connection — do it again at the Gmail step.</p>
      ...
      <h4>Step 3 — fill in, run once</h4>
      <p>Sheets: your Week 3 spreadsheet, sheet <b>Form Responses 1</b>, headers Yes. Gmail: To your own address, Subject New answers, the email text: one column. Right-click the Sheets point → <b>Choose where to start</b> → <b>All</b> → Save — or it only finds new rows. Then <b>Run once</b> — one email per answer. Too many.</p>
      <h4>Step 4 — the point in the middle, your last line, the clock</h4>
      <p>Right-click the dots between them → <b>Add a module</b> → <b>Tools</b> → <b>Text aggregator</b> (source Google Sheets; Advanced settings toggle → row separator New row) and put its <b>Text</b> in the email. Under it, type one line that is only yours: one thing for your body, or one person to contact today.</p>
      ...
      <h4>Stuck?</h4>
      <p>No rows → wrong account, or an empty row in your sheet. No email → check Spam. Ask <b>3 people around you</b> first. Then raise your hand.</p>
    `,
  },
